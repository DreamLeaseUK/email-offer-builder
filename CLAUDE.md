...
**The authoritative technical map is `docs/architecture.md` (Solution Design & System Architecture, current-state).** Current state, decisions and open issues live in `docs/status-2026-09-18.md` (`-09-16.md` / `-09-15.md` / `-09-14.md` are kept for history). **Resuming a session: `docs/PICKUP-PROMPT.md`** (rewritten end of session 3, 16 Sept — the definitive hand-off). **Brochure discovery is the finder (built 18 Sept 2026, no allowlist): the design, the decisions and the evidence are in `docs/brochure-finder-brief.md`.**
...
- Firecrawl is the only metered service. Direct fetch first, Firecrawl fallback, cache for 24 hours. A brochure search caps at 25 credits (typically 4–16) and runs once per model per 90 days.
...
packages/adapters  Source and output adapter interfaces (§5.2) and implementations: url lookup (normalise, HTMLRewriter page parser, site pricing JSON, offer assembly), Firecrawl client, brochure finder/harvest/manual/ensure. Pure: I/O is injected.
