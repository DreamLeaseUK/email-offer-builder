...
Paste everything below the line into a new Claude Code session opened in `C:\Users\MatthewWilson\email-offer-builder`. **Rewritten 15 September 2026 (end of session 2).** It supersedes all earlier pickup prompts. Read it top to bottom before doing anything.
...
- **Git:** `main`, clean tree, **pushed to `origin`** = `https://github.com/DreamLeaseUK/email-offer-builder` (private, DreamLeaseUK org). Auth is via Git Credential Manager (stored) — `git push` just works. Run `git log --oneline -8` for the latest. Key recent commits: `70aadd6` (Firecrawl rawBase64 download), `e0a2072` (Compose: audience/salsac/brochure UI), `15878a4` (salsac render), `5f40928` (interim allowlist fixes), then the session-2 handoff docs. **Working tree is clean — nothing uncommitted was left behind.**
- **Tests:** `pnpm test` → **130 pass** (schema 18, adapters 52, render 23, api 37). `pnpm typecheck` clean.
- **Live Worker:** https://offer-mailer.matt-wilson-9b8.workers.dev/health → v0.3.0. **Prod was NOT redeployed this session — the live Worker is behind HEAD.** Every `/api` route returns **503 in production until Cloudflare Access is configured (IT dependency)**, so the tool is exercised via `wrangler dev` + tests, not live `/api`.
- **Firecrawl key:** now present in `apps/api/.dev.vars` as `FIRECRAWL_API_KEY` (local dev; `wrangler dev` health shows `firecrawl:true`). `.dev.vars` is git-ignored — the key is not in the repo. Whether the prod Worker secret is set is unconfirmed.
- **Dev servers** (won't survive the session): `cd apps/api && pnpm dev` (Worker on :8787) and `pnpm --filter @offer-mailer/web dev` (Vite; picks the next free port, was :5174). Vite proxies `/api`,`/c`,`/r`,`/f`,`/b`,`/a` → :8787. Vite binds to `localhost` (IPv6) — curl `http://localhost:PORT`, not `127.0.0.1`.
...
| 4 Web app (Compose/Campaigns/Library/Register) | **Core screens built, dev-only** (Vite+React, vendored design system). Not yet served from the prod Worker. |
...
| 7 Template admin, approval, register, suppression | Register (in-app + CSV) done; **template admin + Emma approval + suppression list still to build** |
| 8 Stubs + `docs/evolution.md` | Not started |

## 4. What was built in session 2 (15 Sept, this session)

All committed & pushed. Key additions:

- **Audience type + salary sacrifice** (Compose): a selector for **PCH / BCH / Salary sacrifice**, wired to the offer's `contractType` so it drives the compliance block, terms and disclaimer. Salary sacrifice = a looked-up (personal) vehicle URL converted to a salsac offer: **no initial payment**, two hand-entered net prices (**20% / 40% taxpayer**), price "includes finance, maintenance and insurance". Two salsac compliance blocks already exist as placeholders (real wording is Emma's, TBD). (`e0a2072`, `15878a4`)
- **Include-brochure UI** (Compose `BrochureControl`): "Add brochure" (harvest), manual upload/paste fallback, include toggle. (`e0a2072`)
- **Firecrawl `rawBase64` download** (`70aadd6`): manufacturer CDNs (Akamai) 403 the Worker's direct PDF fetch; Firecrawl's `fetchFile` returns the original bytes so we can host any brochure. **Keep this — it's load-bearing for the redesign.**
- **Interim brochure-allowlist fixes** (`5f40928`): forced https on gated URLs (was 500-ing previews), taught the matcher to read UK content on global domains (`/uk/` path segment), corrected 6 stale entries. **These are interim — the whole allowlist approach is being replaced (see §5).**
...
- **IT:** Cloudflare Access (unblocks prod `/api`; today it 503s) + custom domains (`mailer.` / `offers.dreamlease.co.uk`) + the Graph Entra app (for "Create draft in Outlook").
- **Emma:** the real approved compliance wording for the three contract types (PCH/BCH/salary sacrifice) — the templates currently carry placeholders.
...
Confirm in a few lines that you've read the docs above, state the git + test + live state as you find them (`git log`, `pnpm test`, `/health`), and confirm whether Matt wants to (a) wait for Fable on the brochure redesign, (b) push on step 7 (template admin + Emma approval + suppression), or (c) something else. Propose nothing else until he answers.
